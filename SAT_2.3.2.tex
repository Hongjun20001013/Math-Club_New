...
% 封面信息
\title[SAT Unit 2.3.2 Exponential Functions]{\textbf{SAT Unit 2.3.2\\ Exponential Functions}}
...
\small
\textbf{Parent function:} \(f(x) = 3^x\) \quad (gray curve in each picture below)

\vspace{0.2cm}
\begin{center}
    \includegraphics[width=0.72\linewidth]{E25.png}
\end{center}

\vspace{0.15cm}
\begin{itemize}
    \item[\textbf{a.}] \(g(x)=3^{x+1}\) — \textbf{shift left} 1 unit
    \item[\textbf{b.}] \(h(x)=3^x-2\) — \textbf{shift down} 2 units
    \item[\textbf{c.}] \(k(x)=-3^x\) — \textbf{flip} across the \(x\)-axis
    \item[\textbf{d.}] \(j(x)=3^{-x}\) — \textbf{flip} across the \(y\)-axis
\end{itemize}
\end{frame}

\begin{frame}{Transformations of the Exponential Function — Quick Rules}
\small
Start with \(f(x)=3^x\).

\vspace{0.2cm}
\begin{enumerate}
    \item[\textbf{Inside the exponent}] \(f(x+1)=3^{x+1}\) \\
    Replace \(x\) with \(x+1\) \(\Rightarrow\) graph moves \textbf{left} 1 unit.
    
    \item[\textbf{Outside minus a number}] \(f(x)-2=3^x-2\) \\
    Subtract 2 from the output \(\Rightarrow\) graph moves \textbf{down} 2 units.
    
    \item[\textbf{Negative in front}] \(-f(x)=-3^x\) \\
    Multiply outputs by \(-1\) \(\Rightarrow\) \textbf{reflection across the \(x\)-axis}.
    
    \item[\textbf{Negative input}] \(f(-x)=3^{-x}\) \\
    Replace \(x\) with \(-x\) \(\Rightarrow\) \textbf{reflection across the \(y\)-axis}.
\end{enumerate}

\vspace{0.15cm}
\textbf{Memory trick:} changes \textbf{inside} the function move the graph \textbf{left/right}; changes \textbf{outside} move it \textbf{up/down}.
\end{frame}
...
W(t) = 830(1.09)^{\frac{6}{8}t}
\]
The investment increases by $9\%$ each time the exponent increases by $1$. Let $m$ be the number of months. Since $t = \frac{m}{12}$:
\[
