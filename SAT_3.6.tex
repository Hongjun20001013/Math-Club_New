...
\begin{frame}{Inference from Samples}
\small

Use a \textbf{random sample} to estimate a \textbf{population} we cannot measure fully.

\vspace{0.2cm}
\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Statistic} (sample) & known --- e.g. $\hat{p} = 65\%$ \\
\hline
\textbf{Parameter} (population) & unknown --- true rate for everyone \\
\hline
\end{tabular}
\end{center}
...
\vspace{0.2cm}
\begin{itemize}
    \item Larger sample $\Rightarrow$ better estimate (smaller MOE)
    \item Results are \textbf{plausible}, not guaranteed
\end{itemize}
...
\begin{frame}{Margin of Error (MOE)}
\small

\textbf{Margin of error} = how far a \textbf{sample result} might be from the \textbf{true population value}.

\vspace{0.2cm}
\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Sample estimate} & from your data (e.g. 52\%) \\
\hline
\textbf{Population parameter} & true value for everyone (unknown) \\
\hline
\end{tabular}
\end{center}
...
\vspace{0.15cm}
\begin{itemize}
    \item \textbf{Not this:} The survey is ``wrong by MOE.''
    \item \textbf{This:} Truth is probably within MOE of the estimate.
\end{itemize}

\vspace{0.05cm}
\textbf{SAT:} sample \% = \textbf{center} of a range, not an exact fact.

\end{frame}

\begin{frame}{How to Read MOE}
\small

\textbf{Poll:} 1{,}000 voters — \textbf{52\%} support Candidate A, MOE \textbf{$\pm 3\%$}
...
\vspace{0.1cm}
\begin{itemize}
    \item \textbf{52\%} = sample estimate \hfill \textbf{49–55\%} = plausible range for all voters
    \item Outside the range (e.g.\ 35\%, 60\%) $\Rightarrow$ \textbf{doubtful}
\end{itemize}

\vspace{0.1cm}
\begin{block}{Target analogy}
Arrow at \textbf{52}; bullseye (truth) likely in \textbf{49–55}, not exactly at 52.
\end{block}

\end{frame}

\begin{frame}{What Affects MOE?}
\small

\begin{center}
\begin{tabular}{|l|c|}
\hline
\textbf{Change} & \textbf{MOE} \\
\hline
Larger sample size ($n$) & Smaller $\downarrow$ \\
\hline
Higher confidence level & Larger $\uparrow$ \\
\hline
More population variability & Larger $\uparrow$ \\
\hline
\end{tabular}
\end{center}

\vspace{0.25cm}
\textbf{On the SAT:} bigger MOE $\Rightarrow$ check for a \textbf{smaller sample} first.
...
\vspace{0.1cm}
\begin{itemize}
    \item \textbf{A–C:} misread MOE (sample mislabel, ``impossible,'' or picks a bound only)
    \item \textbf{D:} 35\% is \textbf{outside} the range $\Rightarrow$ doubtful
\end{itemize}

\textbf{Answer:} \boxed{D}
...
\textbf{Answer:} \textbf{D} — Sample A had a smaller sample size.

\vspace{0.1cm}
\small
Same population, same method $\Rightarrow$ larger MOE means \textbf{smaller} $n$.
\end{frame}
...
\vspace{0.1cm}
\begin{itemize}
    \item \textbf{A:} wrong range (misses 61\%)
    \item \textbf{B:} MOE is not about individuals in the sample
    \item \textbf{C:} 60\% is \textbf{outside} the range $\Rightarrow$ unlikely
    \item \textbf{D:} outside, but ``not possible'' is too strong
\end{itemize}
...
\begin{frame}{Answer}
\textbf{Answer:} D — Sample B had \textbf{fewer} customers (smaller $n$ $\Rightarrow$ larger MOE).
\end{frame}
