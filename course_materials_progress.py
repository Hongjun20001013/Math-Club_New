...
def ap_calc_progress_from_progress(
    progress: dict[str, Any], slide_count: int, challenge_count: int
) -> int:
    """Slide progress from completed challenges — not passive slide views."""
    if challenge_count > 0:
        done = len(progress.get("done") or [])
        return min(100, round(100 * done / challenge_count))
    if slide_count <= 0:
        return 0
    done = len(progress.get("done") or [])
    return min(100, round(100 * done / slide_count))


def ap_calc_mastery_from_progress(progress: dict[str, Any], slide_count: int, checkpoint_count: int) -> int:
    """AP Calc mastery: scored challenges/checkpoints only — lab browsing does not count."""
    cp = progress.get("checkpoint") or {}
    if cp.get("last_run") and isinstance(cp["last_run"], dict):
        return min(100, int(cp["last_run"].get("pct") or 0))
    if cp.get("best_total"):
        return min(100, round(100 * int(cp.get("best_score") or 0) / int(cp["best_total"])))
    return 0
