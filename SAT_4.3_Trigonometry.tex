...
\begin{frame}{\textbf{What If the Angle is Greater Than 90 Degrees?}}
\small
...
\textbf{Big idea:} for angles outside a right triangle, do not start by drawing a new triangle.  
Start with the angle's position on the coordinate plane.

\bigskip

\textbf{Use this 4-step routine:}
\begin{enumerate}
    \item Draw the angle in \textbf{standard position}: start on the positive \(x\)-axis and rotate counterclockwise.
    \item Identify the \textbf{quadrant} where the terminal side lands.
    \item Find the \textbf{reference angle}: the acute angle between the terminal side and the \(x\)-axis.
    \item Use the reference-angle value, then attach the correct sign.
\end{enumerate}
...
\[
\begin{array}{c|c}
\text{Quadrant} & \text{Positive trig ratio} \\
\hline
I & \sin,\ \cos,\ \tan \\
II & \sin \\
III & \tan \\
...
\begin{frame}{\textbf{Example: Angle Greater Than 90 Degrees}}

\textbf{Find \(\sin(150^\circ)\).}

\bigskip

\begin{enumerate}
    \item \(150^\circ\) lands in \textbf{Quadrant II}.
    \item The reference angle is \(180^\circ - 150^\circ = 30^\circ\).
    \item \(\sin(30^\circ)=\dfrac{1}{2}\).
    \item Sine is positive in Quadrant II.
\end{enumerate}
...
\small
\textbf{What the unit circle gives us:}

\medskip

Put angle \(\theta\) in standard position and look at the point where the terminal side hits the circle.  
If that point is \((x,y)\), then:
...
\textbf{So when solving a unit-circle problem:}
\begin{enumerate}
    \item Reduce the angle to a familiar terminal position.
    \item Find the point \((x,y)\) on the unit circle.
    \item Read \(\sin\) from \(y\), read \(\cos\) from \(x\), and use \(y/x\) for \(\tan\).
\end{enumerate}

\begin{center}
    \includegraphics[width=0.38\linewidth]{E66.png}
\end{center}
\end{frame}
