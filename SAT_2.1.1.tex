...
% 封面信息
\title[SAT Unit 2.1.1]{\textbf{SAT Unit 2.1.1 \\Exponents \& Radicals}}
...
\section{Exponents}
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \fill[white, opacity=0.3] (current page.center) circle (4cm);
        \node[align=center] at (current page.center) {
            {\huge\bfseries\textcolor{purpleblue}{Exponents}} \\[0.5cm]
            {\large\itshape\textcolor{lightText}{Excellence in SAT Prep}}
        };
        \foreach \x in {1,2,3,4,5} {
            \fill[purpleblue, opacity=0.2] (rand*10-5, rand*7-3.5) circle (0.1+\x*0.05);
...
\begin{frame}{Laws of Exponents (Part 1)}
    ...
    \begin{itemize}
        \item \textbf{Identity:} \(x^1 = x\) \text{\small (e.g., \(3^1 = 3\))}
        \item \textbf{Zero Exponent:} \(x^0 = 1\) \text{\small (e.g., \(3^0 = 1\))}
    \end{itemize}

    \vspace{0.4cm}
    \textbf{Multiplication and Division Rules}
    \begin{itemize}
        \item \textbf{Product Rule:} \(x^m \cdot x^n = x^{m+n}\) \text{\small (e.g., \(3^4 \cdot 3^5 = 3^9\))}
        \item \textbf{Quotient Rule:} \(\dfrac{x^m}{x^n} = x^{m-n}\) \text{\small (e.g., \(\dfrac{3^7}{3^3} = 3^4\))}
    \end{itemize}
\end{frame}
...
\begin{frame}{Laws of Exponents (Part 2)}
    \textbf{Power Rules}
    \begin{itemize}
        \item \textbf{Power of a Power:} \((x^m)^n = x^{mn}\) \text{\small (e.g., \((3^2)^4 = 3^8\))}
        \item \textbf{Power of a Product:} \((xy)^m = x^m y^m\) \text{\small (e.g., \((2 \cdot 3)^3 = 2^3 \cdot 3^3\))}
        \item \textbf{Power of a Quotient:} \(\left(\dfrac{x}{y}\right)^m = \dfrac{x^m}{y^m}\) \text{\small (e.g., \(\left(\dfrac{2}{3}\right)^3 = \dfrac{2^3}{3^3}\))}
    \end{itemize}

    \vspace{0.4cm}
    \textbf{Negative Exponent}
    \begin{itemize}
        \item \textbf{Rule:} \(x^{-m} = \dfrac{1}{x^m}\) \text{\small (e.g., \(3^{-4} = \dfrac{1}{3^4}\))}
    \end{itemize}
\end{frame}
...
\begin{frame}{\textbf{Question: Product of Exponential Expressions}}
    ...
    \vspace{0.5em}
    \begin{itemize}
        \item[\textbf{A.}] \(b^{14} c^{-5} + c^2 d^{-10}\)
        \item[\textbf{B.}] \(b^{14} c^{-5} + c^2\)
        \item[\textbf{C.}] \(b^{14} c^{-5} d^{-5} + b^6 c^2\)
        \item[\textbf{D.}] \(b^{14} c^{-5} d^{-5} + c^2\)
    \end{itemize}
\end{frame}

\begin{frame}{\textbf{Answer Explanation: Product of Exponential Expressions}}
    ...
    \[
    b^6 c^{-2} d^{-5} \cdot b^8 c^{-3} = b^{14} c^{-5} d^{-5}
    \]
    \[
    b^6 c^{-2} d^{-5} \cdot c^4 d^5 = b^6 c^2
    \]
...
\begin{frame}
\section{Radicals \& Roots}
    \begin{tikzpicture}[remember picture, overlay]
        \shade[top color=softPurple, bottom color=softGray]
            (current page.north west) rectangle (current page.south east);
        \fill[white, opacity=0.3] (current page.center) circle (4cm);
        \node[align=center] at (current page.center) {
            {\huge\bfseries\textcolor{purpleblue}{Radicals \& Roots}} \\[0.5cm]
            {\large\itshape\textcolor{lightText}{Excellence in SAT Prep}}
        };
        \foreach \x in {1,2,3,4,5} {
            \fill[purpleblue, opacity=0.2] (rand*10-5, rand*7-3.5) circle (0.1+\x*0.05);
...
\begin{frame}{Rationalizing Denominators}
    ...
    \[
    \dfrac{1}{\sqrt{2} + 1} \cdot \dfrac{\sqrt{2} - 1}{\sqrt{2} - 1} = \dfrac{\sqrt{2} - 1}{(\sqrt{2})^2 - 1^2} = \sqrt{2} - 1
    \]
\end{frame}

\begin{frame}{\textbf{Question: Simplifying an Exponential Expression}}
...
\begin{frame}{\textbf{Answer Explanation: Simplifying an Exponential Expression}}
    \[
    = \left( \dfrac{27}{81} \cdot x^{6-2} \cdot y^{3-9} \right)^{\frac{1}{3}}
    = \left( \dfrac{1}{3} \cdot x^4 \cdot y^{-6} \right)^{\frac{1}{3}}
    = \dfrac{x^{\frac{4}{3}}}{3 y^2}
    \]

    \textbf{Correct Answer:} \( \boxed{\dfrac{x^{4/3}}{3y^2}} \)
\end{frame}
...
\begin{frame}{\textbf{Question: Rational Exponents and Roots}}
    ...
    \]
    for all positive values of \(x\), what is the value of \(\dfrac{a}{b}\)?
\end{frame}

\begin{frame}{\textbf{Answer Explanation: Rational Exponents and Roots}}
    ...
    \textbf{Correct Answer:} \( \boxed{\dfrac{7}{6}} \)
\end{frame}
...
\begin{frame}{\textbf{Answer Explanation: Exponent and Radical Expression Equivalence}}
    We begin with:
    \[
    \sqrt[5]{70n \left( \sqrt[6]{70n} \right)^2}
    ...
    We are told this is equal to \((70n)^{30x}\), so:
    \[
    30x = \frac{4}{15} \Rightarrow x = \frac{4}{450} = \frac{2}{225}
    \]

    \textbf{Correct Answer:} \( \boxed{\dfrac{2}{225}} \)
\end{frame}
...
\begin{frame}{\textbf{Question: Solving for \(k\)}}
    If \(\left(5^3\right)^{4k} = \left(5^{\frac{1}{3}}\right)^{24}\), what is the value of \(k\)?
\end{frame}

\begin{frame}{\textbf{Answer Explanation: Solving for \(k\)}}
...
\begin{frame}{\textbf{Question: Equivalent Exponential Expression}}
...
\begin{frame}{\textbf{Answer Explanation: Equivalent Exponential Expression}}
...
\begin{frame}{\textbf{Question: Nested Radicals and Exponents}}
    \[
...
\begin{frame}{\textbf{Answer Explanation: Nested Radicals and Exponents}}
    \textbf{Correct Answer:} \( \boxed{\dfrac{7}{120}} \)
\end{frame}
...
\begin{frame}{\textbf{Answer Explanation: Simplifying Radical Expressions}}
    Numerator:
    \[
    10 \cdot \sqrt[6]{9^3 \cdot x^{48}} = 10 \cdot (9^3 \cdot x^{48})^{1/6} = 10 \cdot 9^{1/2} \cdot x^8 = 30x^8
    \]
    ...
    So \(a = 5\), \(b = \dfrac{31}{4}\), and \(ab = \dfrac{155}{4}\).

    \textbf{Correct Answer:} \( \boxed{\dfrac{155}{4}} \)
\end{frame}

\begin{frame}{\textbf{Question: Combining Radical Expressions}}
    \[
...
\begin{frame}{\textbf{Answer Explanation: Combining Radical Expressions}}
    \[
    \sqrt[4]{x^3} \cdot \sqrt[12]{x^5} = x^{\frac{3}{4}} \cdot x^{\frac{5}{12}} = x^{\frac{9}{12} + \frac{5}{12}} = x^{\frac{14}{12}} = x^{\frac{7}{6}} = \sqrt[6]{x^7}
    \]

    Therefore, \(m = 7\), \(n = 6\), and \(\dfrac{m}{n} = \dfrac{7}{6}\).
