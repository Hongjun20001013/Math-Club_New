...
\begin{itemize}
    \item Adding the extreme low value 10 to data set F \textbf{lowers the mean}, so $\text{mean of G} < \text{mean of F}$. The mean is \textbf{not} less for F than for G.
    \item The \textbf{median} is unchanged: 10 is far below the range in set F (170–290), so it does not shift the middle value when all 56 values are ordered.
\end{itemize}

\textbf{Conclusion:} Neither the mean nor the median is less for data set F than for data set G.
...
\begin{itemize}
    ...
    \item The \textbf{mean} and \textbf{sum} are unchanged: every increase of 7 is matched by a decrease of 7.
