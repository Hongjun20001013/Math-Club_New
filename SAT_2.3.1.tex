...
% 封面信息
\title[SAT Unit 2.3.1]{\textbf{SAT Unit 2.3.1\\ Polynomial and Rational Functions}}
...
\begin{frame}{Polynomial Long Division — Example}
\small

\textbf{Problem:} Divide
\[
2x^3 + 3x^2 - 5x + 6
\]
by \(x - 2\).
...
\begin{array}{rl}
  & 2x^2 + 7x + 9 \\
x - 2 \; ) & \overline{2x^3 + 3x^2 - 5x + 6} \\
  & \underline{-2x^3 + 4x^2} \\
  & \phantom{2x^3 + }7x^2 - 5x \\
  & \underline{\phantom{2x^3 + }-7x^2 + 14x} \\
  & \phantom{2x^3 + 3x^2 - }9x + 6 \\
  & \underline{\phantom{2x^3 + 3x^2 - }-9x + 18} \\
  & \phantom{2x^3 + 3x^2 - 5x + }24
\end{array}
\]
\end{minipage}%
...
\begin{frame}{\textbf{Question: Intercepts of a Polynomial Function}}
...
\begin{frame}{\textbf{Answer: Intercepts of a Polynomial Function}}
We are given:
\begin{itemize}
...
The graph cannot cross the $x$-axis where the denominator is zero. Simplify the denominator:
\[
(x - 1)^2 - 4(x - 1) + 4
\]
Let $u = x - 1$, then:
\[
u^2 - 4u + 4 = (u - 2)^2 = (x - 3)^2
\]
So the denominator is zero when $x = 3$.
