...
def _primary(latin: str, cjk: str) -> str:
    return cjk or latin
...
def _draw_table_row(
    ...
    *,
    ...
    _set_font(pdf, primary, font_size, "B" if header else "")
    if header:
...
class _BilingualReportPDF(FPDF):
    ...
    def footer(self) -> None:
        ...
        self.ln(3)
        _set_font(self, self._primary, 7)
        self.set_text_color(*_C_MUTED)
        uw = self.w - self.l_margin - self.r_margin
        self.cell(uw * 0.34, 4, f"{_pdf_brand_name()} Learning Center", align="L")
        self.cell(uw * 0.32, 4, self._header, align="C")
        self.cell(uw * 0.34, 4, f"Page {self.page_no()}/{{nb}}", align="R", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def _section_title(pdf: FPDF, primary: str, en: str, zh: str = "") -> None:
    ...
    _set_font(pdf, primary, 12.5, "B")
    pdf.set_text_color(*_C_NAVY)
    pdf.cell(w * 0.58, 6, en, align="L")
    if zh:
        _set_font(pdf, primary, 10, "B")
        pdf.set_text_color(*_C_DEEP)
        pdf.cell(w * 0.42, 6, zh, align="R", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    else:
        pdf.ln(6)
    y = pdf.get_y()
...
def _metric_card(
    ...
    _set_font(pdf, primary, 13.5, "B")
    pdf.set_text_color(*_C_NAVY)
    pdf.set_xy(x, y + 3.5)
    pdf.cell(w, 7, value[:20], align="C")
    _set_font(pdf, primary, 6.2, "B")
    pdf.set_text_color(*_C_MUTED)
    pdf.set_xy(x, y + 10.5)
    pdf.cell(w, 3.5, label_en, align="C")
    if label_zh:
        pdf.set_xy(x, y + 13.5)
        pdf.cell(w, 3.5, label_zh, align="C")
    pdf.set_text_color(*_C_INK)


def _draw_hero(pdf: FPDF, primary: str, report: dict[str, Any]) -> None:
    w = _content_width(pdf)
    _set_font(pdf, primary, 7, "B")
    pdf.set_text_color(*_C_DEEP)
    pdf.cell(0, 4, "NOVEL PREP LEARNING CENTER · MATHEMATICS PLACEMENT", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(2)
    ...
    _set_font(pdf, primary, 7.5, "B")
    pdf.cell(w - 16, 5, "STUDENT ASSESSMENT REPORT · 学生测试报告", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(pdf.l_margin + 8)
    _set_font(pdf, primary, 15, "B")
    pdf.cell(w - 16, 7, str(report.get("topic_title_en") or ""), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(pdf.l_margin + 8)
    _set_font(pdf, primary, 9.5, "")
    subtitle = _bilingual_block(
        str(report.get("report_subtitle_en") or ""),
        str(report.get("report_subtitle_zh") or ""),
    ).replace("\n", " · ")
    pdf.cell(w - 16, 5, subtitle, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(pdf.l_margin + 8)
    _set_font(pdf, primary, 8.5, "")
    left = f"Student 学生：{report.get('student_name') or 'Student'}"
    right = f"Attempt 编号：{report.get('attempt_id') or ''}"
    pdf.cell((w - 16) * 0.62, 5, left)
    pdf.cell((w - 16) * 0.38, 5, right, align="R")
    pdf.set_text_color(*_C_INK)
...
def _draw_band_table(pdf: FPDF, primary: str, bands: list[dict[str, Any]]) -> None:
    ...
    def _header_row() -> None:
        _draw_table_row(pdf, primary, cols, headers, line_h=3.4, font_size=7.2, header=True, min_h=7)
    ...
    for idx, b in enumerate(bands):
        ...
        _draw_table_row(pdf, primary, cols, cells, line_h=3.5, font_size=6.9, fill=fill, min_h=8)
    pdf.ln(2)
...
def _draw_recommendation(pdf: FPDF, primary: str, rec: dict[str, Any]) -> None:
    ...
    _set_font(pdf, primary, 12, "B")
    pdf.set_text_color(*_C_NAVY)
    pdf.cell(w - 12, 6, f"Recommended Placement: {rec.get('title_en')}", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(pdf.l_margin + 6)
    _set_font(pdf, primary, 10, "")
    pdf.set_text_color(*_C_MUTED)
    pdf.cell(w - 12, 5, f"建议级别：{rec.get('title_zh')}", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    for key in ("narrative_en", "narrative_zh"):
        ...
        if txt:
            _set_font(pdf, primary, 8.5, "")
            pdf.multi_cell(w, 4.2, txt, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(2)
...
def _draw_list_box(
    ...
    pdf.rect(x, y, w, 7.5, "F")
    pdf.set_text_color(*_C_WHITE)
    _set_font(pdf, primary, 8, "B")
    pdf.set_xy(x + 2.5, y + 1.8)
    pdf.cell(w - 5, 5, f"{title_en} · {title_zh}")
    ...
    pdf.set_text_color(*_C_INK)
    pdf.rect(x, y + 7.5, w, body_h, "DF")
    pdf.set_xy(x + 3, y + 10)
    _set_font(pdf, primary, 7.1, "")
...
def _draw_error_patterns(pdf: FPDF, primary: str, patterns: list[dict[str, Any]]) -> None:
    ...
    _draw_table_row(pdf, primary, cols, headers, line_h=3.4, font_size=7.1, header=True, min_h=7)
    ...
    for idx, p in enumerate(patterns[:10]):
        ...
        _draw_table_row(pdf, primary, cols, cells, line_h=3.5, font_size=6.8, fill=fill, min_h=9)
    pdf.ln(2)
...
def _draw_missed_table(pdf: FPDF, primary: str, items: list[dict[str, Any]]) -> None:
    ...
    def _header_row() -> None:
        _draw_table_row(pdf, primary, cols, headers, line_h=3.2, font_size=6.8, header=True, min_h=6.5)
    ...
    for idx, it in enumerate(items):
        ...
        cells = [
            ...
            str(it.get("part") or ""),
            student[:30],
            str(it.get("correct") or "")[:30],
            _bilingual_block(str(it.get("note_en") or ""), str(it.get("note_zh") or "")),
        ]
        _draw_table_row(pdf, primary, cols, cells, line_h=3.2, font_size=6.6, fill=fill, min_h=7)
    pdf.ln(2)
...
def _draw_final_box(pdf: FPDF, primary: str, report: dict[str, Any]) -> None:
    ...
    for key in ("en", "zh"):
        ...
        if txt:
            _set_font(pdf, primary, 8.5, "")
            pdf.multi_cell(w, 4.3, txt, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(2)
    ...
    _set_font(pdf, primary, 8.5, "B")
    pdf.set_text_color(*_C_NAVY)
    pdf.cell(w - 12, 5, "Teacher Summary · 教师总结", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(pdf.l_margin + 6)
    pdf.set_text_color(*_C_INK)
    _set_font(pdf, primary, 8, "")
    pdf.multi_cell(w - 12, 3.8, summary, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
