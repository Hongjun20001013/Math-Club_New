...
Uses measured multi-line table rows so English and Chinese never overlap.
Latin text uses Helvetica (crisp); Chinese uses Noto Sans SC only when needed.
"""
...
_LATIN_SAFE_TRANS = str.maketrans(
    {
        "\u2013": "-",
        "\u2014": "-",
        "\u2212": "-",
...
def _latin_safe(text: str) -> str:
    return text.translate(_LATIN_SAFE_TRANS)


def _setup_bilingual_font(pdf: Any) -> tuple[str, str]:
    """Helvetica for Latin; Noto SC only for Chinese lines."""
...
def _font_for_text(latin: str, cjk: str, text: str, *, bold: bool = False) -> tuple[str, str]:
    if cjk and _has_cjk(text):
        return cjk, ""
    return latin, "B" if bold else ""


def _set_font_for_text(
    pdf: FPDF, latin: str, cjk: str, text: str, size: float, *, bold: bool = False
) -> None:
    fam, style = _font_for_text(latin, cjk, text, bold=bold)
    pdf.set_font(fam, style, size)


def _text_units(ch: str) -> int:
    return 2 if ord(ch) > 127 else 1


def _wrap_text_lines(text: str, col_width_mm: float, font_size_pt: float) -> list[str]:
    if not text:
        return [""]
    char_w_mm = max(1.0, font_size_pt * 0.19)
    max_units = max(4, int(col_width_mm / char_w_mm))
    lines: list[str] = []
    for paragraph in text.split("\n"):
        paragraph = paragraph.strip() or " "
        if _has_cjk(paragraph):
            current = ""
            current_units = 0
            for ch in paragraph:
                units = _text_units(ch)
                if current_units + units > max_units and current:
        ...
        current_units = 0
        for word in words:
            word_units = sum(_text_units(c) for c in word)
            gap_units = 1 if current else 0
            if current_units + gap_units + word_units > max_units and current:
                lines.append(current)
                current = word
                current_units = word_units
            else:
                current = f"{current} {word}".strip()
                current_units = sum(_text_units(c) for c in current)
        if current:
            lines.append(current)
    return lines or [""]
...
def _line_count(text: str, col_width_mm: float, font_size_pt: float) -> int:
    return max(1, len(_wrap_text_lines(text, max(8.0, col_width_mm), font_size_pt)))
...
def _draw_wrapped_text(
    ...
    *,
    ...
    cy = y
    for line in _wrap_text_lines(text, width, font_size):
        fam, style = _font_for_text(latin, cjk, line, bold=bold)
        draw_line = line if fam != latin else _latin_safe(line)
        pdf.set_font(fam, style, font_size)
        pdf.set_xy(x, cy)
        pdf.cell(width, line_h, draw_line, align="L")
        cy += line_h
    return cy
...
def _draw_table_row(
    ...
    *,
    ...
    text_color = _C_WHITE if header else _C_INK
...
class _BilingualReportPDF(FPDF):
    def __init__(self, latin: str, cjk: str, header_label: str) -> None:
    ...
    def footer(self) -> None:
        ...
        _set_font_for_text(self, self._latin, self._cjk, "Learning Center", 8)
        self.cell(uw * 0.34, 4.2, f"{_pdf_brand_name()} Learning Center", align="L")
        _set_font_for_text(self, self._latin, self._cjk, self._header, 8)
        self.cell(uw * 0.32, 4.2, self._header, align="C")
        _set_font_for_text(self, self._latin, self._cjk, "Page", 8)
        self.cell(
            uw * 0.34,
            4.2,
...
def _section_title(pdf: FPDF, latin: str, cjk: str, en: str, zh: str = "") -> None:
    pdf.ln(2)
    w = _content_width(pdf)
    _set_font_for_text(pdf, latin, cjk, en, 13, bold=True)
    pdf.set_text_color(*_C_NAVY)
    pdf.cell(w * 0.58, 6.5, en, align="L")
    if zh:
        _set_font_for_text(pdf, latin, cjk, zh, 11, bold=True)
...
def _metric_card(
    ...
    _set_font_for_text(pdf, latin, cjk, value, 14, bold=True)
    pdf.set_text_color(*_C_NAVY)
    pdf.set_xy(x, y + 3.5)
    pdf.cell(w, 7.5, value[:20], align="C")
    _set_font_for_text(pdf, latin, cjk, label_en, 7, bold=True)
    pdf.set_text_color(*_C_MUTED)
    pdf.set_xy(x, y + 11)
    pdf.cell(w, 3.8, label_en, align="C")
    if label_zh:
        _set_font_for_text(pdf, latin, cjk, label_zh, 7, bold=True)
        pdf.set_xy(x, y + 14.2)
        pdf.cell(w, 3.8, label_zh, align="C")
    pdf.set_text_color(*_C_INK)


def _draw_hero(pdf: FPDF, latin: str, cjk: str, report: dict[str, Any]) -> None:
    w = _content_width(pdf)
    _set_font_for_text(pdf, latin, cjk, "MATHEMATICS PLACEMENT", 7.5, bold=True)
    pdf.set_text_color(*_C_DEEP)
    ...
    _set_font_for_text(pdf, latin, cjk, left, 9)
    pdf.cell((w - 16) * 0.62, 5.2, left)
    _set_font_for_text(pdf, latin, cjk, right, 9)
    pdf.cell((w - 16) * 0.38, 5.2, right, align="R")
    pdf.set_text_color(*_C_INK)
    pdf.set_xy(pdf.l_margin, y0 + box_h + 6)


def _draw_metrics(pdf: FPDF, latin: str, cjk: str, metrics: dict[str, Any]) -> None:
    w = _content_width(pdf)
    gap = 3
    cw = (w - 3 * gap) / 4
    y = pdf.get_y()
    ...
    for val, en, zh, bg in cards:
        _metric_card(pdf, latin, cjk, x, y, cw, h, val, en, zh, bg)
        x += cw + gap
    pdf.set_xy(pdf.l_margin, y + h + 7)


def _draw_band_table(pdf: FPDF, latin: str, cjk: str, bands: list[dict[str, Any]]) -> None:
    _section_title(pdf, latin, cjk, "Score by Readiness Band", "各阶段能力表现")
    w = _content_width(pdf)
    cols = [w * 0.09, w * 0.22, w * 0.1, w * 0.09, w - w * 0.50]
    headers = ["Band", "Area 能力区间", "Score", "Rate", "Interpretation 说明"]

    def _header_row() -> None:
        _draw_table_row(pdf, latin, cjk, cols, headers, line_h=3.8, font_size=8.2, header=True, min_h=8)

    _header_row()
    for idx, b in enumerate(bands):
        if pdf.get_y() > pdf.h - pdf.b_margin - 14:
            pdf.add_page()
            _section_title(pdf, latin, cjk, "Score by Readiness Band (cont.)", "各阶段能力表现（续）")
            _header_row()
        ...
        _draw_table_row(pdf, latin, cjk, cols, cells, line_h=4.0, font_size=8.0, fill=fill, min_h=8.5)
    pdf.ln(2)


def _draw_recommendation(pdf: FPDF, latin: str, cjk: str, rec: dict[str, Any]) -> None:
    _section_title(pdf, latin, cjk, "Placement Recommendation", "分班建议")
    w = _content_width(pdf)
    y0 = pdf.get_y()
    box_h = 30
    ...
    _set_font_for_text(pdf, latin, cjk, "Recommended Placement", 12.5, bold=True)
    pdf.set_text_color(*_C_NAVY)
    pdf.cell(w - 12, 6.2, f"Recommended Placement: {rec.get('title_en')}", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(pdf.l_margin + 6)
    _set_font_for_text(pdf, latin, cjk, str(rec.get("title_zh") or ""), 10.5)
    pdf.set_text_color(*_C_MUTED)
    ...
    for key in ("narrative_en", "narrative_zh"):
        ...
        if txt:
            ...
            end_y = _draw_wrapped_text(
                pdf, latin, cjk, pdf.l_margin, y, w, txt, line_h=4.5, font_size=9
            )
...
def _draw_list_box(
    ...
    pdf.set_text_color(*_C_WHITE)
    _set_font_for_text(pdf, latin, cjk, title_zh, 8.5, bold=True)
    ...
    _draw_wrapped_text(
        pdf, latin, cjk, x + 3, y + 10.2, w - 6, body_text, line_h=line_h, font_size=8
    )
    return y + 7.8 + body_h


def _draw_error_patterns(pdf: FPDF, latin: str, cjk: str, patterns: list[dict[str, Any]]) -> None:
    if not patterns:
        return
    _section_title(pdf, latin, cjk, "Error Pattern Summary", "错误类型总结")
    w = _content_width(pdf)
    cols = [w * 0.24, w * 0.52, w - w * 0.76]
    headers = ["Error Type 错误类型", "Observed Pattern 表现", "Sample Questions 例题"]
    _draw_table_row(pdf, latin, cjk, cols, headers, line_h=3.8, font_size=8, header=True, min_h=8)

    for idx, p in enumerate(patterns[:10]):
        if pdf.get_y() > pdf.h - pdf.b_margin - 12:
            pdf.add_page()
            _section_title(pdf, latin, cjk, "Error Pattern Summary (cont.)", "错误类型总结（续）")
            _draw_table_row(pdf, latin, cjk, cols, headers, line_h=3.8, font_size=8, header=True, min_h=8)
        ...
        _draw_table_row(pdf, latin, cjk, cols, cells, line_h=4.0, font_size=7.8, fill=fill, min_h=9)
    pdf.ln(2)


def _draw_missed_table(pdf: FPDF, latin: str, cjk: str, items: list[dict[str, Any]]) -> None:
    if not items:
        return
    _section_title(pdf, latin, cjk, "Incorrect / Skipped Questions", "错题与空题清单")
    w = _content_width(pdf)
    cols = [w * 0.06, w * 0.07, w * 0.17, w * 0.17, w - w * 0.47]
    headers = ["Q", "Part", "Student", "Correct", "Note 批改说明"]

    def _header_row() -> None:
        _draw_table_row(pdf, latin, cjk, cols, headers, line_h=3.6, font_size=7.8, header=True, min_h=7.5)

    _header_row()
    for idx, it in enumerate(items):
        if pdf.get_y() > pdf.h - pdf.b_margin - 10:
            pdf.add_page()
            _section_title(pdf, latin, cjk, "Incorrect / Skipped (cont.)", "错题清单（续）")
            _header_row()
        fill = _C_ROW_A if idx % 2 == 0 else _C_ROW_B
        ...
        _draw_table_row(pdf, latin, cjk, cols, cells, line_h=3.8, font_size=7.8, fill=fill, min_h=8)
    pdf.ln(2)


def _draw_study_plan(pdf: FPDF, latin: str, cjk: str, plan: list[dict[str, str]]) -> None:
    if not plan:
        return
    _section_title(pdf, latin, cjk, "Recommended Study Plan", "后续学习建议")
    w = _content_width(pdf)
    ...
    for idx, row in enumerate(plan):
        ...
        _draw_table_row(pdf, latin, cjk, cols, cells, line_h=4.0, font_size=8, fill=fill, min_h=8.5)
    pdf.ln(2)


def _draw_final_box(pdf: FPDF, latin: str, cjk: str, report: dict[str, Any]) -> None:
    _section_title(pdf, latin, cjk, "Final Evaluation", "最终评估")
    w = _content_width(pdf)
    ...
    _set_font_for_text(pdf, latin, cjk, "教师总结", 9, bold=True)
    pdf.set_text_color(*_C_NAVY)
...
def build_bilingual_assessment_pdf(report: dict[str, Any]) -> bytes:
    ...
    latin, cjk = _setup_bilingual_font(pdf)
    pdf._latin = latin  # type: ignore[attr-defined]
    pdf._cjk = cjk  # type: ignore[attr-defined]

    pdf.set_title(f"{_pdf_brand_name()} - Placement assessment")
    pdf.alias_nb_pages()
    pdf.add_page()

    _draw_hero(pdf, latin, cjk, report)
    _draw_metrics(pdf, latin, cjk, report.get("metrics") or {})
    _draw_band_table(pdf, latin, cjk, report.get("bands") or [])

    pdf.add_page()
    _draw_recommendation(pdf, latin, cjk, report.get("recommendation") or {})
    _section_title(pdf, latin, cjk, "Strengths and Growth Areas", "优势与待提升点")
    w = _content_width(pdf)
    y_row = pdf.get_y()
    col_w = (w - 4) / 2
    bottom = _draw_list_box(
        pdf, latin, cjk, pdf.l_margin, y_row, col_w,
        "Strengths", "优势", report.get("strengths") or [], "strength",
    )
    bottom2 = _draw_list_box(
        pdf, latin, cjk, pdf.l_margin + col_w + 4, y_row, col_w,
        "Growth Areas", "待提升点", report.get("growth_areas") or [], "growth",
    )
    pdf.set_xy(pdf.l_margin, max(bottom, bottom2) + 5)
    _draw_error_patterns(pdf, latin, cjk, report.get("error_patterns") or [])
    _draw_missed_table(pdf, latin, cjk, report.get("missed_items") or [])

    pdf.add_page()
    _draw_study_plan(pdf, latin, cjk, report.get("study_plan") or [])
    _draw_final_box(pdf, latin, cjk, report)
